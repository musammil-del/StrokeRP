...
- เลือกชุดที่ดีที่สุดจาก F1-macro (CV) มาเป็นโมเดลสุดท้าย พร้อมแสดง Top 3 ชุดที่ดีที่สุด
...
# -----------------------------------------------------------------------
# 4.1) แสดง Top 3 ชุดพารามิเตอร์ที่ดีที่สุด (เรียงตาม F1-macro CV มากไปน้อย)
#      ช่วยให้เห็นว่ามีชุดอื่นที่คะแนนใกล้เคียงกับ best set ไหม เผื่อไว้เทียบ/เลือกเอง
# -----------------------------------------------------------------------
top3_results = sorted(results, key=lambda r: r["f1_macro_cv"], reverse=True)[:3]

print("\n=== Top 3 ชุดพารามิเตอร์ที่ดีที่สุด (เรียงตาม F1-macro CV) ===")
for rank, r in enumerate(top3_results, start=1):
...
# บันทึกผลทุกชุดลงไฟล์ CSV เผื่อไว้เปรียบเทียบ/ทำตาราง (เรียงจากดีสุดไปหาแย่สุด)
...
results_df = results_df.sort_values("f1_macro_cv", ascending=False)
