"""
XGBoost Classifier + SMOTE - Train/Test Split (70/30) + Manual Grid Search (10-fold CV)
================================================================================
...
- ลด max_depth + เพิ่ม reg_alpha/reg_lambda (regularization) เพื่อลด Overfitting
- วนทดสอบทุกชุดพารามิเตอร์เอง (ParameterGrid) พร้อม cross-validation (cv=10)
  ...
  + Recall (macro) + ROC-AUC (ovr, macro) + Accuracy (train)
...
หมายเหตุ: ต้องติดตั้ง xgboost และ imbalanced-learn ก่อนรัน
...
MODEL_PATH = os.path.join(BASE_DIR, "5FSXGB_model.pkl")
...
#    ลด max_depth ลง + เพิ่ม reg_alpha (L1) / reg_lambda (L2) เพื่อลด Overfitting
#    ไม่ใช้ sample_weight="balanced" แล้ว เพราะให้ SMOTE จัดการเรื่อง imbalance แทน
# -----------------------------------------------------------------------
param_grid = {
...
for i, params in enumerate(all_combinations, start=1):
    ...
    # roc_auc_ovr_weighted เก็บไว้ดูเสริม ไม่ใช้เลือกโมเดล (ใช้ f1_macro เป็นหลัก)
